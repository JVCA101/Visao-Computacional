...
def test(dataloader, model, loss_fn, batch_size, epoch, lr, optimizer):
    ...
    try:
        with open('alex_results/alex_' + str(batch_size) + '_' + str(epoch) + '_' + str(lr) + '_' + str(optimizer) + '.txt', 'w') as f:
    ...
    plt.savefig('confusion_matrix/alex_cm_' + str(batch_size) + '_' + str(epoch) + '_' + str(lr) + '_' + str(optimizer) + '.png')
...
#### Testing
start = time.time()
test(test_loader, alexnet, loss_fn, batch_size, (t+1), learning_rate, 'sgd')
end = time.time()
