...
def test(dataloader, model, loss_fn, batch_size, epoch, lr, optimizer):
    ...
    try:
        with open('vgg_results/vgg_' + str(batch_size) + '_' + str(epoch) + '_' + str(lr) + '_' + str(optimizer) + '.txt', 'w') as f:
    ...
    plt.savefig('confusion_matrix/vgg_cm_' + str(batch_size) + '_' + str(epoch) + '_' + str(lr) + '_' + str(optimizer) + '.png')
...
optimizer = torch.optim.SGD(vgg13.parameters(), lr=learning_rate)
...
#### Testing
start = time.time()
test(test_loader, vgg13, loss_fn, batch_size, (t+1), learning_rate, 'sgd')
end = time.time()
